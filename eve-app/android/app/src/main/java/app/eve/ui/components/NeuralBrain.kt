 ...
 *   listening         → LEAN   : the smoke condenses + leans toward you, pulsing with your REAL
 *                                mic level (VoiceState.Hearing.level)
 *   thinking          → BRAIN  : particles snap into the seeded neural graph; synapses fire pulses
 *   speaking          → GENIE  : head + chest + gesturing arms rising from a rotating vapor vortex
 *   working (tool)    → WALKER : a stick figure walking across, dust trailing behind
 *   error/reconnect   → frozen, dimmed haze
 *
 * Every state change BURSTS the particles apart with a glow flash, then springs them into the new
 * shape with a per-particle stagger so it assembles organically (~1s cinematic morph). Physics run
 * in resolution-independent [-1,1] space and scale to the canvas at draw time.
 */

private const val TOTAL = 128                       // particle pool (web parity)
private val RING_COUNTS = intArrayOf(1, 7, 14, 24, 36)
private val RING_RADII = floatArrayOf(0f, 0.24f, 0.46f, 0.7f, 0.94f)
private val GRAPH_N = RING_COUNTS.sum()             // 82 brain neurons; rest are vapor

private enum class Form { SMOKE, LEAN, BRAIN, GENIE, WALKER }

/** The visual state the avatar reads — collapses the conversation machine into the 6 web states. */
private enum class Vis { DISCONNECTED, IDLE, LISTENING, THINKING, SPEAKING, WORKING }

private fun visOf(state: VoiceState, working: Boolean): Vis = when {
    working -> Vis.WORKING
    ...
    state is VoiceState.Error || state is VoiceState.Reconnecting -> Vis.DISCONNECTED
    else -> Vis.IDLE // Idle, Connecting, NoAudio
}

private fun formOf(v: Vis): Form = when (v) {
    Vis.WORKING -> Form.WALKER
...
private val PALETTES: Map<Vis, Palette> = mapOf(
    ...
    Vis.WORKING to Palette(f(52, 211, 153), f(22, 140, 100), f(150, 255, 210), f(16, 185, 129)),
)
...
private fun buildGraph(): Graph {
    ...
    // trunk edges: each node links to the nearest in the previous ring
    ...
    // lateral edges: nearest within the same ring
    for (ring in 1 until RING_COUNTS.size) {
    ...
    // force-directed relax (repulsion + parent spring + radial pull) — once, deterministic
    repeat(120) {
        for (i in 1 until n) {
            var fx = 0f; var fy = 0f
            for (j in 1 until n) {
 ...
 *  until native TTS-RMS / token metering is wired (the seam is here for that). */
...
private class Brain {
    ...
    val blend = floatArrayOf(1f, 0f, 0f, 0f, 0f) // smoke, lean, brain, genie, walker
    private var curForm = Form.SMOKE; private var prevForm = Form.SMOKE; private var morphAt = -10f

    // lerped palette channels (node/edge/pulse/core) x rgb
    ...
    // per-frame walker skeleton (also drawn as bones)
    var bones: Array<Array<FloatArray>> = emptyArray(); private set
    private var walkerHipX = 0f; private var walkerFacing = 1f
    private var walkerHeadC = floatArrayOf(0f, 0f)
    private var walkerSpine = arrayOf(floatArrayOf(0f, 0f), floatArrayOf(0f, 0f))
    private var walkerChains: Array<Array<FloatArray>> = emptyArray()
    private var walkerSkelT = Float.NaN
    ...
    fun update(dt: Float, vis: Vis, sig: Signals) {
        ...
        // palette lerp toward the active state
        ...
        for (i in 0..4) blend[i] += ((if (i == curForm.ordinal) 1f else 0f) - blend[i]) * min(1f, dt * 3.2f)

        // particle dynamics: gaseous forms get loose springs + noise; solid forms snap crisp
        ...
        // advance pulses + decay neuron flash
    ...
    private fun walkerSkeleton() {
        if (phase == walkerSkelT) return
        walkerSkelT = phase
        val cycle = (phase * 0.17f) % 2f
        val posX = (if (cycle < 1f) cycle else 2f - cycle) * 1.0f - 0.5f
        val facing = if (cycle < 1f) 1f else -1f
        val w = phase * 4.6f
        val bob = abs(sin(w)) * 0.035f
        val hipX = posX; val hipY = 0.12f - bob
        val neck = floatArrayOf(hipX, hipY - 0.42f)
        val headC = floatArrayOf(hipX + facing * 0.015f, hipY - 0.54f)
        val sh = floatArrayOf(hipX, hipY - 0.38f)
        val legSwing = sin(w) * 0.5f; val armSwing = sin(w + PI.toFloat()) * 0.45f

        fun limb(ox: Float, oy: Float, swing: Float, upper: Float, lower: Float, bend: Float): Array<FloatArray> {
            val a1 = (PI / 2).toFloat() + swing * facing
            val kx = ox + cos(a1) * upper; val ky = oy + sin(a1) * upper
            val a2 = a1 + bend * facing
            return arrayOf(floatArrayOf(ox, oy), floatArrayOf(kx, ky), floatArrayOf(kx + cos(a2) * lower, ky + sin(a2) * lower))
        }
        val legL = limb(hipX, hipY, legSwing, 0.26f, 0.26f, max(0f, -sin(w)) * 0.8f)
        val legR = limb(hipX, hipY, -legSwing, 0.26f, 0.26f, max(0f, sin(w)) * 0.8f)
        val armL = limb(sh[0], sh[1], armSwing, 0.2f, 0.18f, 0.35f)
        val armR = limb(sh[0], sh[1], -armSwing, 0.2f, 0.18f, 0.35f)
        val spine = arrayOf(neck, floatArrayOf(hipX, hipY))
        walkerHipX = hipX; walkerFacing = facing; walkerHeadC = headC
        walkerSpine = spine; walkerChains = arrayOf(armL, armR, legL, legR)
        bones = arrayOf(spine, legL, legR, armL, armR)
    }

    private fun alongChain(pts: Array<FloatArray>, count: Int, k: Int, jit: (Int) -> Float): FloatArray {
        var total = 0f; val segLen = FloatArray(pts.size - 1)
        for (i in 0 until pts.size - 1) { val l = hypot(pts[i + 1][0] - pts[i][0], pts[i + 1][1] - pts[i][1]); segLen[i] = l; total += l }
        var d = (k.toFloat() / max(1, count - 1)) * total; var seg = 0
        while (seg < segLen.size - 1 && d > segLen[seg]) { d -= segLen[seg]; seg++ }
        val t = if (segLen[seg] > 0f) d / segLen[seg] else 0f
        return floatArrayOf(
            lerp(pts[seg][0], pts[seg + 1][0], t) + (jit(k) - 0.5f) * 0.012f,
            lerp(pts[seg][1], pts[seg + 1][1], t) + (jit(k + 31) - 0.5f) * 0.012f,
        )
    }

    private fun walkerTarget(i: Int) {
        walkerSkeleton()
        when {
            i < 10 -> {
                val ang = (i.toFloat() / 10) * (PI * 2).toFloat() + phase * 0.3f
                tx[i] = walkerHeadC[0] + cos(ang) * 0.095f; ty[i] = walkerHeadC[1] + sin(ang) * 0.095f; glow[i] = 1f
            }
            i < 20 -> {
                val c = alongChain(walkerSpine, 10, i - 10) { ((i + it) % 7).toFloat() / 7f }
                tx[i] = c[0]; ty[i] = c[1]; glow[i] = 0.85f
            }
            i < 52 -> {
                val chain = walkerChains[(i - 20) / 8]
                val c = alongChain(chain, 8, (i - 20) % 8) { ((i + it) % 5).toFloat() / 5f }
                tx[i] = c[0]; ty[i] = c[1]; glow[i] = 0.85f
            }
            else -> {
                val u = (i - 52).toFloat() / (TOTAL - 52)
                tx[i] = walkerHipX - walkerFacing * (0.14f + u * 0.55f) + (s1[i] - 0.5f) * 0.1f
                ty[i] = 0.55f + (s2[i] - 0.5f) * 0.25f - u * 0.1f; glow[i] = 0.25f * (1f - u)
            }
        }
    }
}
...
fun NeuralBrain(
    ...
    val workingNow = rememberUpdatedState(working)
    ...
    LaunchedEffect(reducedMotion) {
        ...
        while (true) {
            withFrameNanos { now ->
                ...
                val vis = visOf(s, workingNow.value)
                // mic level is REAL when we're capturing the user; bot/think are gentle envelopes
                // (a clean seam for wiring native TTS-RMS / token metering later).
    ...
    Canvas(
        ...
        val sizeScale = size.minDimension / 200f
        ...
        val bWalker = brain.blend[Form.WALKER.ordinal]
        ...
        // walker skeleton bones + ground line
        if (bWalker > 0.02f && brain.bones.isNotEmpty()) {
            for (chain in brain.bones) {
                for (j in 1 until chain.size) {
                    drawLine(
                        color = colorOf(brain.pEdge, 0.5f * bWalker),
                        start = Offset(ux(chain[j - 1][0]), uy(chain[j - 1][1])),
                        end = Offset(ux(chain[j][0]), uy(chain[j][1])),
                        strokeWidth = 1.1f, blendMode = BlendMode.Plus,
                    )
                }
            }
            drawLine(
                color = colorOf(brain.pEdge, 0.14f * bWalker),
                start = Offset(ux(-0.8f), uy(0.66f)), end = Offset(ux(0.8f), uy(0.66f)),
                strokeWidth = 1f, blendMode = BlendMode.Plus,
            )
        }
