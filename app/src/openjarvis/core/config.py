...
# $OPENJARVIS_HOME relocates all state (config, DBs, prompts) — the same
# override prompt_loader.py honors. Lets a second instance coexist with
# another install on the same box.
DEFAULT_CONFIG_DIR = Path(os.environ.get("OPENJARVIS_HOME", "~/.openjarvis")).expanduser()
...
class AnalyticsConfig:
    ...
    # Default OFF: a self-contained install must not phone home unless the
    # user opts in (host/key point at the upstream OpenJarvis PostHog).
    enabled: bool = False
