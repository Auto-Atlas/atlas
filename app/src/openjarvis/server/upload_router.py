...
# Resource-exhaustion guards. Files are read with a hard per-file cap so a
# single huge (or zip-bomb-expanding) upload can't blow out memory, and the
# request is bounded by a total-byte and file-count ceiling.
_MAX_FILE_BYTES = 25 * 1024 * 1024  # 25 MB per file
_MAX_TOTAL_BYTES = 100 * 1024 * 1024  # 100 MB per request
_MAX_FILE_COUNT = 50
...
async def ingest_files(
    ...
    if len(files) > _MAX_FILE_COUNT:
        raise HTTPException(
            status_code=413,
            detail=f"Too many files: {len(files)} (max {_MAX_FILE_COUNT}).",
        )
    ...
    for upload in files:
        ...
        # Read with a hard cap (one extra byte detects overflow without
        # buffering the whole oversized payload).
        ...
        if len(data) > _MAX_FILE_BYTES:
            raise HTTPException(
                ...
                detail=(
                    ...
                    f"{_MAX_FILE_BYTES // (1024 * 1024)} MB per-file limit."
                ),
            )
        ...
        if total_bytes > _MAX_TOTAL_BYTES:
            raise HTTPException(
                ...
                detail=(
                    ...
                    f"{_MAX_TOTAL_BYTES // (1024 * 1024)} MB total limit."
                ),
            )

        # Parse content based on extension. PDF/DOCX parsing is CPU-bound and
        # synchronous, so offload it to a thread to avoid blocking the event loop.
        if ext in (".txt", ".md", ".csv"):
            ...
            text = await asyncio.to_thread(_extract_text_from_docx, data)
        else:
            continue
