...
def create_webhook_router(
    ...
    # Webhook routes are exempt from the bearer-auth middleware, so an
    # unconfigured signature secret would leave the route wide open. Fail
    # closed: reject unsigned traffic unless an operator explicitly opts in.
    _allow_unsigned = os.environ.get("OPENJARVIS_ALLOW_UNSIGNED_WEBHOOKS", "") == "1"

    def _reject_unsigned(channel: str) -> Optional[Response]:
        """Return a 503 when *channel* has no secret configured (fail-closed)."""
        if _allow_unsigned:
            return None
        logger.error(
            "%s webhook has no signature secret configured — rejecting "
            "(fail-closed). Configure the secret, or set "
            "OPENJARVIS_ALLOW_UNSIGNED_WEBHOOKS=1 to override.",
            channel,
        )
        return Response(
    ...
    async def twilio_incoming(request: Request) -> Response:
        ...
        if not twilio_auth_token:
            rejected = _reject_unsigned("Twilio")
            if rejected is not None:
                return rejected
        elif not _validate_twilio_signature(
    ...
    async def bluebubbles_incoming(
        ...
        if not bluebubbles_password:
            rejected = _reject_unsigned("BlueBubbles")
            if rejected is not None:
                return rejected
        elif auth != bluebubbles_password:
    ...
    async def whatsapp_incoming(
        ...
        if not whatsapp_app_secret:
            rejected = _reject_unsigned("WhatsApp")
            if rejected is not None:
                return rejected
        else:
    ...
    async def sendblue_incoming(request: Request) -> Response:
        ...
        # A message is only acted on when a channel bridge exists — passed at
        # init or dynamically wired onto app.state at runtime. Resolve it up
        # front so the fail-closed check below covers the case where
        # ``sendblue_channel`` is absent but a dynamic bridge would still process
        # the message.
        active_bridge = bridge or getattr(request.app.state, "channel_bridge", None)

        # Verify webhook secret if configured. Fail closed: if a bridge would
        # process this message but no ``webhook_secret`` is available to verify
        # the signature, reject rather than silently accept a forgeable webhook.
        if sb and sb.webhook_secret:
            ...
            rejected = _reject_unsigned("SendBlue")
            if rejected is not None:
                return rejected
