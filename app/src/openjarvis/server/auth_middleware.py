...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):  # noqa: ANN001
        if self._api_key and self._requires_auth(request.url.path):
            ...
            # Constant-time compare (Spec 6): a plain ``!=`` is timing-attackable.
            # The WS path (websocket_authorized) already uses compare_digest; this
            # closes the HTTP path to match.
            if scheme.lower() != "bearer" or not secrets.compare_digest(token, self._api_key):
                return JSONResponse(
    ...
    def _requires_auth(path: str) -> bool:
        ...
        # OAuth provider redirects cannot carry an Authorization header, so the
        # callback would 401 in API-key mode. It is instead protected by a
        # signed, expiring, browser-bound state cookie (double-submit CSRF).
