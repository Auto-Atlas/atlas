...
try:
    ...
    class CreateEventRequest(_BaseModel):
        """Body for POST /{connector_id}/events — calendar event creation."""

        title: str
        start: str  # 'YYYY-MM-DD HH:MM' (24h, local) or 'YYYY-MM-DD'
    ...
    CreateEventRequest = None  # type: ignore[assignment,misc]
...
def create_connectors_router():
    ...
    async def oauth_start(connector_id: str, request: Request):
        ...
        # CSRF protection: an unguessable state, echoed by the provider and
        # cross-checked against a browser-bound, short-lived cookie on callback.
        ...
        # SameSite=lax so the cookie rides along on the top-level callback
        # redirect; secure when served over https.
        resp.set_cookie(
            key=f"oauth_state_{connector_id}",
            value=state,
            max_age=600,
            httponly=True,
            samesite="lax",
            secure=base_url.startswith("https://"),
            path=f"/v1/connectors/{connector_id}/oauth/callback",
        )
        return resp

    def _oauth_error_page(heading: str, status_code: int):
        """Generic OAuth error page — never reflects untrusted input (XSS-safe)."""
        ...
        return HTMLResponse(
            content=(
                ...
                f"<h2 style='color:#ef4444'>{heading}</h2>"
                "<p>Please return to OpenJarvis and try connecting again.</p>"
    ...
    async def oauth_callback(
        ...
        # CSRF: state must match the browser-bound cookie set in oauth_start.
        cookie_state = ""
        if request is not None:
            cookie_state = request.cookies.get(f"oauth_state_{connector_id}", "")
        if (
            not state
            or not cookie_state
            or not _secrets.compare_digest(state, cookie_state)
        ):
            logger.warning(
                "OAuth callback for %s rejected: state mismatch (CSRF guard)",
                connector_id,
            )
            return _oauth_error_page("Authorization Could Not Be Verified", 400)

        if error:
            # The provider-supplied `error` is untrusted — log it, never reflect it.
            logger.warning("OAuth provider error for %s: %s", connector_id, error)
            return _oauth_error_page("Authorization Failed", 400)
        ...
        try:
            ...
            # Exception text may contain provider response bodies — log, don't reflect.
            logger.error("OAuth token exchange failed for %s: %s", connector_id, exc)
            return _oauth_error_page("Token Exchange Failed", 500)
        ...
        # One-time state — invalidate the cookie now that it's been consumed.
        resp.delete_cookie(
            key=f"oauth_state_{connector_id}",
            path=f"/v1/connectors/{connector_id}/oauth/callback",
        )
    ...
    @router.post("/{connector_id}/events")
    def create_event(connector_id: str, req: CreateEventRequest) -> Dict[str, Any]:
        """Create a calendar event through a connected connector.

        Sync ``def`` on purpose — the connector's HTTP call is blocking, so
        FastAPI runs this in its threadpool (same pattern as trigger_sync).
        """
        _ensure_connectors_registered()
        if not ConnectorRegistry.contains(connector_id):
            raise HTTPException(
        ...
        create = getattr(inst, "create_event", None)
        if not callable(create):
            raise HTTPException(
                status_code=400,
                detail=f"Connector '{connector_id}' does not support event creation",
            )
        ...
        except Exception as exc:  # noqa: BLE001 — surface upstream failures honestly
            logger.error("create_event failed for %s: %s", connector_id, exc)
            raise HTTPException(
                status_code=502, detail=_translate_sync_error(str(exc))
            )
        return {"connector_id": connector_id, "event": event}
