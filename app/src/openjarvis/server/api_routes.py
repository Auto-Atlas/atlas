...
def _serialise_trace(trace) -> dict:
    ...
    for step in d.get("steps", []):
        ...
        # Match the frontend TraceStep shape: per-step duration_ms plus a
        # single `data` object folding input/output/metadata together.
        step["duration_ms"] = round(step.pop("duration_seconds", 0.0) * 1000)
        data: dict = {}
        for key in ("input", "output", "metadata"):
            value = step.pop(key, None)
...
def include_all_routes(app) -> None:
    ...
    # Conversation hub ("second brain") — cross-surface history + typed-chat sync.
    try:
        from openjarvis.server.history_routes import history_router  # noqa: PLC0415

        app.include_router(history_router)
    except Exception:  # never let the hub block the rest of the API
        pass
