...
class ObsidianConnector(BaseConnector):
    ...
    def __init__(
        ...
        # Optional per-user RAG scoping, matched against each file's vault-relative POSIX
        # path. Both empty (the default) = ingest the whole vault, unchanged. include_globs
        # restricts ingestion to matching files (e.g. ["project-*.md", "index.md", "TODO.md"]);
        # exclude_globs drops matching files (e.g. ["archive/**", "daily/**"]). Exclude wins.
        self._include_globs: List[str] = list(include_globs) if include_globs else []
        self._exclude_globs: List[str] = list(exclude_globs) if exclude_globs else []
        self._connected: bool = bool(vault_path) and Path(vault_path).is_dir()
        self._items_synced: int = 0
        self._items_total: int = 0

    def _passes_globs(self, rel_posix: str) -> bool:
        """Whether a vault-relative path survives the optional include/exclude scoping.

        No globs configured -> everything passes (whole-vault ingest, backward-compatible).
        ``**`` segments are normalized to ``*`` so ``archive/**`` excludes the whole subtree."""
        def _match(pat: str) -> bool:
            return fnmatch(rel_posix, pat) or fnmatch(rel_posix, pat.replace("**", "*"))

        if self._include_globs and not any(_match(p) for p in self._include_globs):
            return False
        if self._exclude_globs and any(_match(p) for p in self._exclude_globs):
            return False
        return True
        ...
        for root, dirs, files in os.walk(vault):
            ...
            for filename in files:
                ...
                if fpath.suffix.lower() not in _TEXT_EXTENSIONS:
                    continue
                if not self._passes_globs(fpath.relative_to(vault).as_posix()):
                    continue
