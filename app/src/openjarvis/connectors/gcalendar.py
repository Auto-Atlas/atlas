...
def _parse_start_value(raw: str) -> tuple:
    """Parse an event start string into ``(datetime, is_date_only)``.

    Accepts ``YYYY-MM-DD HH:MM[:SS]`` (a ``T`` separator also works) or a
    bare ``YYYY-MM-DD`` for all-day events. Raises :class:`ValueError` for
    anything else — callers surface that as a validation error, never a
    silently-wrong date on someone's calendar.
    """
    ...
    for fmt, date_only in (
        ...
        try:
            ...
            continue
    raise ValueError(
        f"Unparseable start {raw!r} — use 'YYYY-MM-DD HH:MM' or 'YYYY-MM-DD'."
    )
...
class GCalendarConnector(BaseConnector):
    ...
    def create_event(
        ...
        *,
        ...
        """Create a calendar event via ``events.insert`` and return a summary.

        The consent scope granted at connect time is the full
        ``https://www.googleapis.com/auth/calendar`` (see ``GOOGLE_ALL_SCOPES``),
        so no re-consent is needed for writes.

        Parameters
        ----------
        title:
            Event summary. Required, non-empty.
        start:
            ``'YYYY-MM-DD HH:MM'`` (24h, local) or ``'YYYY-MM-DD'``.  A bare
            date implies an all-day event.
        end:
            Optional explicit end in the same format; defaults to
            ``start + duration_min``.
        duration_min:
            Event length in minutes when *end* is omitted (clamped 5–1440).
        all_day:
            Force an all-day event even if *start* carries a time.
        calendar_id:
        ...
        -------
        dict
            ``{"id", "htmlLink", "summary", "start", "end", "status"}`` from
            the created event resource.

        Raises
        ------
        ValueError
            On a missing title or unparseable start/end.
        RuntimeError
            When not authenticated.
        httpx.HTTPStatusError
            When Google rejects the insert (after one 401 refresh retry).
        """
        title = (title or "").strip()
        if not title:
            raise ValueError("Event title must not be empty.")

        start_dt, date_only = _parse_start_value(start)
        all_day = all_day or date_only

        try:
            duration_min = max(5, min(24 * 60, int(duration_min)))
        ...
        if all_day:
            # Google's all-day `end.date` is EXCLUSIVE — a one-day event on
            # July 3 spans start=07-03, end=07-04.
            end_dt = (
                _parse_start_value(end)[0] if end else start_dt
            ) + timedelta(days=1)
            body["start"] = {"date": start_dt.strftime("%Y-%m-%d")}
            body["end"] = {"date": end_dt.strftime("%Y-%m-%d")}
        else:
            if end:
                end_dt = _parse_start_value(end)[0]
                if end_dt <= start_dt:
                    raise ValueError("Event end must be after its start.")
            else:
                end_dt = start_dt + timedelta(minutes=duration_min)
            # Naive local datetimes -> aware via the system zone so Google
            # gets an explicit UTC offset instead of guessing a timezone.
            start_aware = start_dt.astimezone()
            end_aware = end_dt.astimezone()
            body["start"] = {"dateTime": start_aware.isoformat()}
            body["end"] = {"dateTime": end_aware.isoformat()}

        if not self.is_connected():
            raise RuntimeError("Google Calendar not authenticated")

        created = call_with_refresh(
        ...
        return {
            ...
            "start": created.get("start", body["start"]),
            "end": created.get("end", body["end"]),
