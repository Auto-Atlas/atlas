...
import { useJarvisBridgeState } from './JarvisBridgeContext';
...
export function InputArea() {
  ...
  // When the voice sidecar is live it owns the mic continuously, so the button
  // becomes a listen/speaking indicator instead of push-to-talk. Push-to-talk
  // (useSpeech) stays as the fallback whenever the sidecar isn't connected.
  const voice = useJarvisBridgeState();
          ...
          <div className="flex items-center gap-1">
            {voice.connected ? (
              <div
                ...
                title={
                  voice.botSpeaking
                    ? 'Jarvis is speaking — talk to interrupt (barge-in)'
                    : `Live voice on${voice.mode ? ` (${voice.mode})` : ''} — the sidecar is listening`
                }
                style={{
                  color: voice.botSpeaking ? 'var(--color-accent)' : '#22c55e',
                  animation: voice.botSpeaking ? 'pulse 1.2s ease-in-out infinite' : 'none',
                }}
              >
                <Radio size={16} />
              </div>
