...
import { apiFetch, getBase, isTauri } from '../lib/api';
import {
...
function SendBlueSection({
  ...
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [health, setHealth] = useState<{ channel_connected: boolean; bridge_wired: boolean; ready: boolean } | null>(null);
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', padding: '12px 14px' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--color-success)' }}>
              Active &mdash; text {(cfg.from_number as string) || 'your number'} to chat
            </div>
          </div>
          <button
        ...
        {health && (
          <div style={{
            ...
            Channel: {health.channel_connected ? 'connected' : 'not connected'}
            {' \u2022 '}Bridge: {health.bridge_wired ? 'wired' : 'not wired'}
