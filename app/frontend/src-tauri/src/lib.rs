...
/// Phase 2: launch the local voice sidecar (jarvis-sidecar) that owns the mic,
/// runs STT -> LLM -> TTS locally, and streams transcripts + per-turn metrics to
/// the UI over ws://127.0.0.1:8765. Fire-and-forget: the React bridge auto-reconnects,
/// so if the sidecar isn't installed the app still runs — the live-voice panel just
/// stays disconnected. Override the folder with the JARVIS_SIDECAR_DIR env var.
#[cfg(target_os = "windows")]
fn spawn_jarvis_sidecar() {
    // No owner-specific default: without JARVIS_SIDECAR_DIR we look for a
    // sibling "atlas" checkout next to the user's home, then give up loudly.
    let dir = match std::env::var("JARVIS_SIDECAR_DIR") {
        Ok(d) => d,
        Err(_) => {
            let home = std::env::var("USERPROFILE").unwrap_or_default();
            let guess = std::path::Path::new(&home).join("atlas");
            if !guess.join(r".venv\Scripts\python.exe").exists() {
                eprintln!(
                    "jarvis-sidecar: JARVIS_SIDECAR_DIR is not set and {} has no venv — skipping voice sidecar. UI runs without live voice.",
                    guess.display()
                );
                return;
            }
    ...
    let python = std::path::Path::new(&dir).join(r".venv\Scripts\python.exe");
    if !python.exists() {
        eprintln!(
            "jarvis-sidecar: {} not found — skipping voice sidecar (set JARVIS_SIDECAR_DIR or create the venv). UI runs without live voice.",
            python.display()
        );
        return;
    }
    match std::process::Command::new(&python)
        ...
        .spawn()
    {
        Ok(child) => eprintln!("jarvis-sidecar: spawned pid {} from {dir}", child.id()),
        Err(e) => eprintln!("jarvis-sidecar: failed to spawn from {dir}: {e}"),
    }
}

/// The voice sidecar needs Windows-native audio + the NVIDIA stack, so it's a
/// no-op elsewhere. The UI still runs; the live-voice bridge simply never connects.
#[cfg(not(target_os = "windows"))]
fn spawn_jarvis_sidecar() {}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Phase 2: launch the local voice sidecar (owns the mic; streams
            // transcripts + per-turn metrics to the UI on ws://127.0.0.1:8765).
            spawn_jarvis_sidecar();
