...
# ---------------------------------------------------------------------------
# create_event — calendar WRITE via events.insert (mocked API)
# ---------------------------------------------------------------------------


def _connect(connector, tmp_path: Path) -> None:
    """Write a fake token file at the connector's credentials path."""
...
def test_create_event_timed(mock_insert, connector, tmp_path: Path) -> None:
    """A timed event posts summary + offset-aware start/end to events.insert."""
    _connect(connector, tmp_path)
    mock_insert.return_value = {
        "id": "new1",
        "htmlLink": "https://calendar.google.com/event?eid=new1",
        "summary": "Fireworks",
        "start": {"dateTime": "2026-07-03T21:00:00-05:00"},
        "end": {"dateTime": "2026-07-03T22:00:00-05:00"},
        "status": "confirmed",
    }

    result = connector.create_event("Fireworks", "2026-07-03 21:00")

    (token, calendar_id, body) = mock_insert.call_args[0]
    assert token == "fake-access-token"
    assert calendar_id == "primary"
    assert body["summary"] == "Fireworks"
    # Local-naive input becomes offset-aware RFC3339 (Google needs the offset)
    assert body["start"]["dateTime"].startswith("2026-07-03T21:00:00")
    assert "+" in body["start"]["dateTime"] or "-" in body["start"]["dateTime"][10:]
    assert body["end"]["dateTime"].startswith("2026-07-03T22:00:00")  # default 60 min
    assert result["id"] == "new1"
    assert result["htmlLink"].endswith("eid=new1")


@patch("openjarvis.connectors.gcalendar._gcal_api_event_insert")
def test_create_event_all_day_end_exclusive(mock_insert, connector, tmp_path: Path) -> None:
    """A bare date creates an all-day event with Google's exclusive end date."""
    _connect(connector, tmp_path)
    mock_insert.return_value = {"id": "new2", "status": "confirmed"}

    connector.create_event("Founders Day", "2026-07-03")
...
@patch("openjarvis.connectors.gcalendar._gcal_api_event_insert")
def test_create_event_explicit_end_and_extras(mock_insert, connector, tmp_path: Path) -> None:
    """Explicit end wins over duration; description/location pass through."""
    _connect(connector, tmp_path)
    mock_insert.return_value = {"id": "new3"}

    connector.create_event(
        "Standup",
        "2026-07-06 09:00",
        end="2026-07-06 09:15",
        duration_min=60,
    ...
    body = mock_insert.call_args[0][2]
    assert body["end"]["dateTime"].startswith("2026-07-06T09:15:00")
    assert body["description"] == "Daily sync"
    assert body["location"] == "Shop"


def test_create_event_validation_errors(connector, tmp_path: Path) -> None:
    """Empty title, bad start, and inverted end all raise ValueError."""
    _connect(connector, tmp_path)
    with pytest.raises(ValueError):
        connector.create_event("", "2026-07-03 21:00")
    with pytest.raises(ValueError):
        connector.create_event("X", "tomorrow at 9")
    with pytest.raises(ValueError):
        connector.create_event("X", "2026-07-03 21:00", end="2026-07-03 20:00")


def test_create_event_not_connected(connector) -> None:
    """Without credentials, create_event refuses before any network call."""
    with pytest.raises(RuntimeError):
        connector.create_event("Fireworks", "2026-07-03 21:00")
