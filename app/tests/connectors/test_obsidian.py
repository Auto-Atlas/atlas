...
# ---------------------------------------------------------------------------
# Test 10: per-user RAG scoping — include/exclude globs ("don't dig too deep")
# ---------------------------------------------------------------------------


def test_no_globs_ingests_whole_vault(vault: Path) -> None:
    """Default (no globs configured) ingests the whole vault — backward-compatible."""
    from openjarvis.connectors.obsidian import ObsidianConnector  # noqa: PLC0415

    conn = ObsidianConnector(vault_path=str(vault))
    assert len(list(conn.sync())) == 3


def test_include_globs_restricts_ingest(vault: Path) -> None:
    """include_globs limits ingestion to matching vault-relative paths."""
    from openjarvis.connectors.obsidian import ObsidianConnector  # noqa: PLC0415

    conn = ObsidianConnector(vault_path=str(vault), include_globs=["note1.md"])
    docs = list(conn.sync())
    assert [d.doc_id for d in docs] == ["obsidian:note1.md"]


def test_exclude_globs_drops_subtree(vault: Path) -> None:
    """exclude_globs drops matching files; '**' drops a whole subtree (exclude wins)."""
    from openjarvis.connectors.obsidian import ObsidianConnector  # noqa: PLC0415

    conn = ObsidianConnector(vault_path=str(vault), exclude_globs=["subdir/**"])
    ids = sorted(d.doc_id for d in conn.sync())
    assert ids == ["obsidian:note1.md", "obsidian:note2.md"]
